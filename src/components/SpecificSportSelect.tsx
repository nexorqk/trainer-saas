...
const SpecificSportSelect = ({
  ...
  setSpecificSport: (sport: string) => void;
}) => {
  const sportSpecifics = useQuery(api.sport_specifics.get);

  return (
    <Select.Root value={specificSport} onValueChange={setSpecificSport}>
      <Select.Trigger
        className="inline-flex items-center justify-center rounded px-[15px] text-[13px] leading-none h-[35px] gap-[5px] bg-white text-violet11 shadow-[0_2px_10px] shadow-black/10 hover:bg-mauve3 focus:shadow-[0_0_0_2px] focus:shadow-black data-[placeholder]:text-violet9 outline-none"
        aria-label="Food"
      >
        <Select.Value placeholder="Выберите спецификацию" />
...
export default SpecificSportSelect;
